var RolesPermissions = require('../models/model-rolePermissioning')
...
module.exports = function (server) {
    ...
    server.delete('/deleteAllRolePermissions', (req, res) => {
        RolesPermissions.destroy({ where: { role_id: req.body.value } })
            .then((rolePermisions) => {
                // console.log(rolePermisions);
                res.json({ success: true, data: rolePermisions, message: 'Permissions for this role removed successfully.' })
            })
            .catch((err) => {
                res.json({ success: false, err: err })
            })
